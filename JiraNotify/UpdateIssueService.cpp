...
STDMETHODIMP CUpdateIssueService::OnRun(IVariantObject* pResult)
{
    ...
    { //Read
        ...
        RETURN_IF_FAILED(Diff(pSavedIssues, pServerIssues, pResult, newIds));
    }
    ...
    { //Write
        RETURN_IF_FAILED(pResult->SetVariantValue(KEY_ISSUES, &CComVar(pServerIssues)));
        RETURN_IF_FAILED(pResult->SetVariantValue(KEY_ISSUES_NEW_KEYS, &CComVar(StrJoin(L";", newIds))));
    }
...
void CUpdateIssueService::HandleJiraError(IJiraConnection *pJiraXmlRpcConnection, IVariantObject* pResultObject)
{
    ...
    pResultObject->SetVariantValue(KEY_HAS_MESSAGE, &CComVar(bstr));
}

HRESULT CUpdateIssueService::Diff(IJiraObjectsCollection* pFirstCollection, IJiraObjectsCollection* pSecondCollection, IVariantObject* pResultObject, TStringVector& vectorNewIds)
{
    ...
    if (result.size())
    {
        RETURN_IF_FAILED(pResultObject->SetVariantValue(KEY_HAS_CHANGES, &CComVar(true)));
        vectorNewIds = result;
    }
    return S_OK;
}
